...
class BayesianDecisionTree:
    ...
    def _split(self, X, y):
        ...
        for feature in range(n_features):
            ...
            if len(weights) > 0:
                ...
                valid_thresholds = np.array(valid_thresholds)  # 确保阈值数组是 NumPy 数组
                if total_weight > 0:
                    probabilities = weights / total_weight
                    mean = np.sum(probabilities * valid_thresholds)  # 使用有效阈值
                    variance = np.sum(probabilities * (valid_thresholds - mean) ** 2)
                    std_dev = np.sqrt(variance)
                else:
                    mean = np.mean(valid_thresholds)  # 使用有效阈值
    ...
    def _predict_sample(self, x, node, sigma=0):
        """Predict a single sample using the tree."""
        if node["type"] == "leaf":
            return node["class"]

        threshold = node["threshold_dist"].rvs() + np.random.normal(0, sigma)  # Add noise to threshold
        if x[node["feature"]] <= threshold:
            return self._predict_sample(x, node["left"], sigma)
        else:
            return self._predict_sample(x, node["right"], sigma)

    def predict(self, X, n_samples=10, sigma=0):
        """Predict with Bayesian sampling over thresholds."""
        predictions = []
        for _ in range(n_samples):
            sample_preds = [self._predict_sample(x, self.tree, sigma) for x in X]
...
if __name__ == "__main__":
    ...
    # Evaluate accuracy for different noise levels
    accuracies = []
    for sigma in np.arange(0, 0.51, 0.1):
        y_pred = bdt.predict(X_test, n_samples=100, sigma=sigma)
        accuracy = accuracy_score(y_test, y_pred)
        accuracies.append(accuracy)
        print(f"Sigma: {sigma:.2f}, Test Accuracy: {accuracy:.4f}")
