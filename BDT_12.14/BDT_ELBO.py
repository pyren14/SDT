...
if __name__ == "__main__":
    ...
    accuracies = []
    for n_samples in range(0, 101, 10):
        if n_samples == 0:
            accuracies.append(0.0)  # No samples, no accuracy
            continue
        y_pred = bdt.predict(X_test, n_samples=n_samples)
        accuracy = accuracy_score(y_test, y_pred)
        accuracies.append(accuracy)
        print(f"Number of Samples: {n_samples}, Test Accuracy: {accuracy:.4f}")
