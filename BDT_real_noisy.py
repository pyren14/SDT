...
class BayesianDecisionTree:
    ...
    def _split(self, X, y):
        ...
        for feature in range(n_features):
            ...
            valid_thresholds = []  # count split point
            ...
            if len(weights) > 0:
                ...
                valid_thresholds = np.array(valid_thresholds)  # make sure threshold is numpy
                if total_weight > 0:
                    probabilities = weights / total_weight
                    mean = np.sum(probabilities * valid_thresholds)  # use valid threshold
    ...
    def _predict_sample(self, x, node, noise_std=0.2):
        ...
        # add gaussian noise
        noisy_x = x.copy()
        noisy_x += np.random.normal(loc=0, scale=noise_std, size=x.shape)

        # sample split point from gaussian distribution
        threshold = node["threshold_dist"].rvs()
        if noisy_x[node["feature"]] <= threshold:
            return self._predict_sample(noisy_x, node["left"], noise_std)
        else:
            return self._predict_sample(noisy_x, node["right"], noise_std)

    def predict(self, X, n_samples=10, noise_std=0.1):
        """Predict with Bayesian sampling over thresholds and added noise."""
        predictions = []

        # doing inference
        for _ in range(n_samples):
            sample_preds = [self._predict_sample(x, self.tree, noise_std) for x in X]
            predictions.append(sample_preds)

        predictions = np.array(predictions)

        # take average as the prediction
...
if __name__ == "__main__":
    ...
    y_pred = bdt.predict(X_test, n_samples=100, noise_std=0.2)
    accuracy = accuracy_score(y_test, y_pred)
    print(f"Test Accuracy on MNIST with noise: {accuracy:.4f}")
